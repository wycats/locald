...
/// Verifies the canonical Toolbx/Distrobox workflow:
///
/// 1. Host: `sudo locald admin setup` (installs setuid shim)
/// 2. Container: `locald up` (runs services, tolerates missing shim)
///
/// This test uses sandbox mode to run in isolation. While sandbox mode
/// skips shim verification entirely (so we can't test the warning messages),
/// it validates that the overall workflow succeeds.
///
/// The container detection logic itself is unit-tested in utils.rs.
/// This test ensures the end-to-end `locald up` flow works.
#[test]
fn test_container_workflow_in_sandbox() {
    let mut ctx = TestContext::new();
    ...
    let mut env_vars: Vec<(String, String)> = ctx
        ...
        .collect();
    env_vars.push(("container".to_string(), "oci".to_string()));

    // Create a simple project
    let project_dir = ctx.root.path().join("container-test");
    fs::create_dir(&project_dir).unwrap();
...
"#;
    ...
    // Start daemon with container env
    ctx.start_daemon();

    // Run `locald up` in the "container" environment
    // This should succeed (exit 0) in sandbox mode
    ctx.command()
        .envs(env_vars.clone())
        .arg("up")
        .arg(&project_dir)
