...
function renderServices() {
  if (state.services.length === 0) {
    el.serviceList.innerHTML =
      '<div style="padding:1rem; color:#666;">No services running</div>';
    return;
  }

  el.serviceList.innerHTML = state.services
    .map(
      (s) => `
        ...
        s.status
            ...
            <div class="service-details">
                ...
                ${
                  s.url
                    ? `<br><a href="${s.url}" target="_blank" class="service-link">${s.url}</a>`
                    : ""
                }
            </div>
        </div>
...
function renderLog(entry) {
  ...
  const isNearBottom =
    el.logViewer.scrollHeight -
      el.logViewer.scrollTop -
      el.logViewer.clientHeight <
    100;
  if (isNearBottom) {
