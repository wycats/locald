...
| StatusNotifier/AppIndicator tray host | Experimental desktop integration | Linux `locald tray ...` desktop status surface                  | Tray agent unavailable for the current desktop session       | `locald tray start` fails with an explicit host diagnostic; `locald tray status` reports the current shell host readiness | Start from a graphical desktop session with D-Bus; on GNOME, install or enable AppIndicator/StatusNotifier support |
...
- The desktop tray is optional. Missing tray-host support does not block the core `locald up` workflow, but it does make `locald tray start` fail because a successful tray start must produce a visible desktop status item.
