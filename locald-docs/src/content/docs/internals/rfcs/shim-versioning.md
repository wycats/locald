...
4.  **Strict Enforcement**: If the versions do not match, `locald` aborts the operation.
    - In interactive sessions, it attempts to auto-fix by running `sudo locald admin setup`.
    - Otherwise, it instructs the user to run `sudo locald admin setup` manually.
