...
- **Phase Activation**:
  - Identify the ID of the phase you are starting from `exo status`, `exo plan review`, or the output of `exo context restore`.
  - Run \`exo phase start <id>\` to mark the phase as active in the plan.
- **Plan Alignment**:
  - Consult `exo plan review` and `exo phase read-details` to identify the **Epoch** and **Phase** goals.
  - **RFC Selection**: Identify which Stage 1+ RFCs this phase implements. Add them to the plan (e.g., `rfcs = ["0030"]`).
  - Update the current implementation plan artifact to be completely focused on the next phase, deriving the high-level goals from `exo` output. Ask the user for feedback.
  - Ensure the Implementation Plan includes a **User Verification** section for manual checks.
  - Use `exo task add` to initialize the active task list from the implementation plan.
