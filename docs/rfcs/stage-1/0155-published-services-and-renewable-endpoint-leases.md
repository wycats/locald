...
7. Acquisition installs the lease only in a registry transition serialized with configuration application, project pause and Resume, and the wake barrier. It revalidates the exact acquisition snapshot and pause state immediately before commit and admits that tuple as the lease's declaration authority. If the project configuration generation changed, acquisition restarts server-side resolution against the new complete snapshot; it never commits authority validated against an older generation. If pause commits first, acquisition may install only a paused `probe_only` scope with no route authorization; if acquisition commits first, the pause transaction immediately fences and suppresses it. If Resume commits first, acquisition observes the unpaused state; if acquisition commits first while paused, Resume replaces its paused scope before probing. The commit returns the stable semantic origin, redacted opaque lease handle, and publisher-private binding revision, then begins locald-observed health evaluation.
...
A lease is active only while its current monotonic deadline is strictly later than the current lease-clock value. Deadline expiry is authoritative even before the cleanup task runs; the reaper removes state but does not define when authority ends. Renewal compares the lease identity fence and atomically confirms the lease is still active before advancing only the renewal revision and monotonic deadline. An expiry task removes a lease only after atomically confirming that its complete expiry fence is still current and that the current deadline has elapsed. A timer captured before a successful renewal therefore cannot expire the renewed lease.
