...
10. Rebind validates and probes a candidate capability, then compare-and-swaps the binding and returns its new publisher-private binding revision. A failed candidate is closed by locald and preserves the last healthy upstream. A successful swap retains the new capability, invalidates the old binding and its traffic scope, closes binding-scoped idle connection pools, and cancels binding-scoped work. The old listener capability remains retained until every request, probe, stream, upgrade bridge, and other task that could initiate or reuse upstream I/O has quiesced. Only then may locald release the final old capability guard.
...
The session nonce makes only an exact acquisition replay idempotent: the same principal, nonce, resolved `ServiceKey`, and listener-capability identity return the existing handle and binding revision after confirming that lease is still current. Replay does not renew the deadline, change the binding or traffic scope, clear pause, update health, or otherwise mutate lifecycle state. Reusing that nonce with another service, listener identity, or request is rejected. While any lease is active, every acquisition with another nonce is rejected even when it comes from the same principal; renewal preserves it and endpoint changes use expected-revision rebind. After expiry, release, takeover, or configuration invalidation, a nonce already consumed in the current daemon epoch cannot establish another lease generation; reacquisition requires a fresh nonce. The daemon retains spent-nonce replay protection only for its current epoch, and restart creates a new epoch with no live lease or replay state. A different publisher may take over immediately only after the daemon proves the previous publisher process is gone; otherwise it waits for expiry or an explicit future handoff mechanism. Successful takeover creates a new lease generation.
...
- same-user acquire, exact-binding readiness wait, renew, rebind, and release IPC with ownership-preserving loopback-listener transfer;
...
- retry-stable acquisition is idempotent only for an exact principal, nonce, service, request, and listener replay; replay does not extend the deadline or alter binding, scope, pause, or health; another nonce from the same active principal is rejected and must use rebind; reusing a nonce with another listener or after lease retirement cannot create a generation; and concurrent different publishers produce one winner;
...
- acquisition and renewal set their deadline to their own commit time plus TTL; renewal never adds TTL to the prior deadline; wall-clock jumps do not affect lease authority; suspend time counts against the monotonic deadline; wake before or after a deadline, while paused, during Resume, configuration transfer or revocation, or candidate rebind serializes before acting on the registry and fences all pre-suspend work; every still-active binding is reproved in a fresh probe-only scope; and inability to establish suspend-inclusive elapsed time retires all publication authority, terminates exact waits, and requires fresh-nonce reacquisition;
