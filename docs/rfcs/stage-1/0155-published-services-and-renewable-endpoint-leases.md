...
Projects that do not opt in keep Exo's current direct-loopback behavior. An Exo installation that cannot discover a compatible locald publication API at all also retains that compatibility path, even when the repository already contains a published declaration. This decision belongs to Exo before locald participates: the launch result must identify the direct loopback origin, and locald itself never degrades its standard networking contract or claims that fallback as a locald service. Once Exo discovers a compatible locald API and resolves the published declaration, failed preparation, publication, health, or origin authorization returns actionable locald setup or recovery guidance and never silently switches away from the selected stable origin. A user who deliberately wants locald's constrained unprivileged behavior must still select sandbox mode explicitly.
...
- Exo still returns a visibly direct-loopback launch when the project has not opted in or no compatible locald publication API can be discovered; after Exo discovers that API and resolves the declaration, publication, health, and origin-authorization failures remain on the stable-origin path and return actionable guidance without silent fallback.
