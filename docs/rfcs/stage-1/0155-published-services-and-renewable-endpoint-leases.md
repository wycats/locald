...
- **Acquisition snapshot**: the exact project configuration generation plus the resolved published declaration's service type, primary semantic origin, and effective health-policy revision under which acquisition began.
...
- **Route authorization**: immutable eligibility for one exact declaration authority, `routable` traffic-scope revision, binding revision, and health-policy revision. It is created only by an atomic promotion from a successfully committed health result or by a serialized declaration-authority transfer that proves the routing-relevant declaration unchanged. It cannot be inherited by another scope, origin, or policy.
...
```text
AcquirePublishedEndpoint(observed_daemon_epoch, project_locator, service_name, session_nonce, listener_capability)
RenewPublishedEndpoint(lease_handle)
RebindPublishedEndpoint(lease_handle, expected_binding_revision, rebind_nonce, listener_capability)
...
1. Every operation obtains the Unix-socket peer identity from the kernel. The authenticated IPC handshake also returns the daemon's random current epoch; acquisition echoes that observed epoch, and a daemon rejects a request from any other epoch before resolving or mutating project state.
...
4. Acquisition resolves the locator server-side, verifies the exact project instance, requires `type = "published"` for the named service, and captures the acquisition snapshot used by the rest of the operation.
...
7. Acquisition completes the standard-mode domain bootstrap before installing a lease or returning a semantic origin. Registration may publish the persistent declaration and claims first, but acquisition and ordinary configuration application share one host-convergence serialization boundary. Within it, acquisition revalidates the configuration generation immediately before atomically synchronizing the complete current host set through the normal privileged setup contract, then records that exact generation as host-synchronized. An older generation therefore either writes before a newer configuration convergence—which then wins—or observes the newer generation and performs no stale write. A synchronization failure preserves or restores the last complete mapping, leaves the declaration in `waiting_for_publisher`, installs no lease, and returns actionable `sudo locald admin setup` or claim-conflict guidance; it is never reduced to a logged warning. After synchronization, acquisition installs the lease only in a registry transition serialized with configuration application, project pause and Resume, and the wake barrier. It revalidates the exact acquisition snapshot, pause state, and matching host-synchronized generation immediately before commit and admits that tuple as the lease's declaration authority. If the project configuration generation changed during bootstrap, acquisition restarts server-side resolution and required synchronization against the new complete snapshot; it never commits authority validated against an older generation. If pause commits first, acquisition may install only a paused `probe_only` scope with no route authorization; if acquisition commits first, the pause transaction immediately fences and suppresses it. If Resume commits first, acquisition observes the unpaused state; if acquisition commits first while paused, Resume replaces its paused scope before probing. The commit returns the stable semantic origin, redacted opaque lease handle, publisher-private binding revision, and server-selected relative renewal schedule, then begins locald-observed health evaluation.
8. The owning workflow uses the bounded, cancelable lease-handle-and-binding-revision-scoped readiness operation before returning a user-facing launch URL. It succeeds only while that exact lease and binding remain active and possess current route authorization for the currently admitted declaration authority, primary origin, and health policy. It remains pending through checking, observed unhealthiness, ordinary renewal, an alias-only or unrelated configuration transfer, and a health-policy reprobe. It terminates on timeout, cancellation, pause, lease expiry or release, binding replacement, declaration removal, type or primary-origin change, or daemon-epoch change; it never observes a successor lease or binding as success. Normal status remains redacted and is not a substitute for this authority-scoped wait. Renewal never self-attests readiness.
9. Renewal extends only the exact current lease and returns its replacement relative renewal schedule. It does not revive an expired lease, change a binding, clear project pause, or restore routing while paused.
10. Rebind validates and probes a candidate capability, then compare-and-swaps the binding and returns its new publisher-private binding revision. Each attempt carries a fresh rebind nonce so an exact retry can recover a committed response without bypassing expected-revision comparison. A failed candidate is closed by locald and preserves the last healthy upstream. A successful swap retains the new capability, invalidates the old binding and its traffic scope, closes binding-scoped idle connection pools, and cancels binding-scoped work. The old listener capability remains retained until every request, probe, stream, upgrade bridge, and other task that could initiate or reuse upstream I/O has quiesced. Only then may locald release the final old capability guard.
...
The protocol uses five purpose-specific fences:

- the **acquisition fence** is the acquisition snapshot that must still match at lease-install commit;
...
- the **health fence** adds to the binding fence the exact declaration authority, health-policy revision, and traffic-scope revision under which a probe began;
- the **expiry fence** adds the renewal revision and expiry deadline to the lease identity fence.

A lease deadline exists only in daemon memory and is measured against one daemon-lifetime, suspend-inclusive monotonic clock, never wall-clock `SystemTime`. The acquisition deadline is the acquisition commit time plus the server-selected TTL; every successful renewal replaces it with the renewal commit time plus that TTL, rather than adding TTL to the old deadline. Route selection, health commit, wait, renew, rebind, release, and expiry all consult this same clock. Wall-clock correction cannot extend or shorten publication authority. On wake, a serialized registry barrier runs before publication operations, route selection, or existing traffic scopes may perform upstream I/O: it establishes elapsed suspend time, reads the suspend-inclusive clock, expires elapsed leases, cancels every pre-suspend traffic and candidate scope, and gives each still-active binding a fresh `probe_only` scope with no route authorization. The barrier cannot race configuration transfer or revocation, pause or Resume, acquisition, or rebind commit against a stale registry snapshot. An unexpired publisher therefore keeps its lease but must pass health again; an expired publisher must reacquire. If the platform or a wake transition cannot establish trustworthy suspend-inclusive elapsed time, locald retires every live publication lease, binding, traffic scope, candidate, and route authorization. Authority-scoped waits terminate as lease lost, unpaused projects return `waiting_for_publisher`, paused projects remain `route_paused` and become `waiting_for_publisher` only after explicit Resume, and publishers must reacquire with fresh nonces. locald never substitutes wall time or preserves authority with an unknowable deadline.
...
Every successful acquisition and renewal response includes publisher-private relative `renew_after` and `expires_in` durations computed from the current monotonic deadline when the response is serialized. The server chooses `renew_after` strictly before expiry with enough policy-defined margin for ordinary scheduling jitter; the exact values remain Stage 2 details. Before sending either request, the publisher records its own monotonic request-start instant and schedules the next renewal no later than `request_start + renew_after`; if that instant has already passed when the response arrives, it renews immediately. This conservatively charges IPC delay against the returned margin without comparing clocks across processes. Publishers never infer the policy from wall-clock timestamps or hard-coded intervals. A lost renewal response is retried with the same lease handle: if that exact lease remains active, the retry performs an ordinary fresh renewal whose deadline is its own commit time plus TTL and returns a new schedule; if authority has actually expired or retired, it fails as lease lost. An exact acquisition replay recomputes the remaining schedule without extending the deadline. While the lease and original acquisition binding remain active, insufficient normal margin produces `renew_after = 0` plus the true positive `expires_in`; acquisition replay reports expiry only after the monotonic deadline has elapsed or the lease has been atomically retired.

The observed daemon epoch and session nonce make only an exact acquisition replay idempotent: the same current epoch, principal, nonce, resolved `ServiceKey`, and listener-capability identity return the existing handle and the original acquisition binding revision after confirming that both the lease and that binding remain current. Replay never substitutes a later rebind's revision; after binding replacement it fails as superseded. Replay does not renew the deadline, change the binding or traffic scope, clear pause, update health, or otherwise mutate lifecycle state. Reusing that nonce with another service, listener identity, or request is rejected. While any lease is active, every acquisition with another nonce is rejected even when it comes from the same principal; renewal preserves it and endpoint changes use expected-revision rebind. After expiry, release, takeover, or configuration invalidation, replay fails as lease lost and a nonce already consumed in the current epoch cannot establish another lease generation; reacquisition requires a fresh nonce. Restart creates a new daemon epoch with no live lease or replay state. A retry that echoes the prior epoch is therefore rejected as daemon replaced before its nonce can be interpreted; the publisher must accept the new authenticated handshake and acquire with a fresh nonce. A different publisher may take over immediately only after the daemon proves the previous publisher process is gone; otherwise it waits for expiry or an explicit future handoff mechanism. Successful takeover creates a new lease generation.

A rebind nonce is scoped to one exact lease generation and expected binding revision. Before candidate work begins, locald reserves one attempt record keyed by the exact lease generation, expected revision, nonce, and candidate listener identity. The first request owns the single in-flight probe and compare-and-swap; an exact duplicate joins that attempt and receives its one terminal result, while mismatched reuse is rejected immediately. A successful binding compare-and-swap and its terminal success record commit in the same registry transition; a terminal failure is likewise recorded before joined callers are released. A joined or later replay returns that recorded result without renewing the lease, reproving health, or changing any scope. If the committed binding revision is still current, replay returns that installed revision; if a later rebind, expiry, release, configuration retirement, or daemon restart superseded its authority, replay fails as superseded or lease lost rather than reporting another binding. Reusing the nonce with a different expected revision, listener identity, or request is rejected. A recorded failure replays without another probe; another attempt requires a fresh nonce, and a retired lease's nonce can never affect its successor.

Health results compare the health fence and confirm that the current lease is still active at commit time; ordinary renewal does not make an otherwise current health result stale. Changing the effective health policy advances both the health-policy and traffic-scope revisions, cancels the prior scope, marks the binding `checking_endpoint`, and makes it ineligible for routing until a probe in the replacement `probe_only` scope succeeds. A result from an older policy or canceled scope cannot commit after that transition. A successful current probe may authorize routing only through an atomic promotion transaction that verifies its complete health fence, active lease, and declaration authority, observes the project unpaused, advances the traffic-scope revision, installs the successor `routable` scope, and creates route authorization keyed to that successor. Advancing the revision makes every other callback from the source scope stale; health evidence is never copied outside this verified transaction. A candidate rebind probe runs in its own `probe_only` scope and captures the current health-policy revision, declaration authority, and pause state. Every project pause or Resume transition cancels all candidate scopes, so a candidate begun across either boundary cannot commit. The rebind commit compares the lease identity fence, expected binding revision, captured declaration authority, candidate scope identity, and pause state, then reconfirms that the lease and candidate scope remain active. If unpaused, it atomically installs the candidate binding, its fresh `routable` traffic scope, and route authorization derived from that exact successful candidate result. If paused, it installs the candidate binding with a fresh `probe_only` scope and no route authorization. A candidate probed under a replaced declaration authority, canceled scope, changed pause state, or expired lease cannot commit. Release and process-exit reaping compare the exact current lease identity.

Configuration application and acquisition bootstrap share the host-convergence boundary; configuration application, lease installation, project pause and Resume, and the wake barrier share the registry transition boundary. Host convergence writes only the complete set for the generation revalidated immediately before the write and records successful synchronization for that generation, so failed or canceled older work cannot supersede a newer mapping. Each live lease and route authorization carries the admitted configuration generation, published type, exact primary origin, and effective health-policy revision. A route may be selected only when that declaration authority matches the current configuration and the concrete host is its canonical primary origin. Configuration transitions apply these rules atomically:
...
- Declaration removal, a change away from `type = "published"`, or a primary-origin change retires the lease, binding, scopes, and authorization. Removing and re-adding textually identical configuration is a new declaration authority and cannot revive the old lease or nonce.

Delayed invalidation compares the generation and declaration then current, so older work cannot revoke a newly installed or transferred lease. locald never exposes the old binding under a new origin. The publisher must reacquire, receive the new origin, pass health again, and reauthorize its application before routing resumes. Any delayed work tied to an endpoint also compares its binding revision. A stale or expired publisher cannot regain authority, mutate a successor, or publish delayed health for it.
...
| Lease expires or publisher dies | Stable unavailable surface; `waiting_for_publisher`, or `route_paused` while the project remains paused | Never signaled | Publisher starts a new lease generation |
...
- publisher UID, PID, process birth, executable, session nonce, or rebind nonce;
...
3. Exo acquires or renews the worktree's published service lease and schedules its next renewal from the returned publisher-private relative schedule.
...
5. Exo uses its lease handle and expected binding revision to wait for that exact binding to become ready; replacement or rebind fails this launch attempt rather than silently observing a successor.
...
- an ephemeral per-service publisher registry, suspend-inclusive monotonic lease clock, wake barrier, and deterministic clock-driven tests;
- same-user acquire, exact-binding readiness wait, renew, retry-idempotent rebind, and release IPC with ownership-preserving loopback-listener transfer and same-network-namespace proof;
- acquisition bootstrap through the existing atomic domain-claim and required hosts-synchronization contract;
...
- first-use acquisition publishes the complete domain claims and finishes every required host mapping before success; acquisition and configuration application serialize host convergence, revalidate the generation before each complete-set write, and record the synchronized generation; a host-synchronization failure or canceled stale retry preserves the last complete mapping, installs no lease, and returns actionable setup or conflict guidance; and config-change-during-sync ordering never lets an older generation overwrite a newer one;
- retry-stable acquisition is idempotent only for an exact observed daemon epoch, principal, nonce, service, request, and listener replay; replay returns the original acquisition revision only while that binding remains current, fails as superseded after rebind, and recomputes the remaining relative renewal schedule without extending the deadline or altering binding, scope, pause, or health; another nonce from the same active principal is rejected and must use rebind; reusing a nonce with another listener or after lease retirement cannot create a generation; an old-epoch retry after daemon restart is rejected before its nonce can become a fresh acquisition; and concurrent different publishers produce one winner;
- acquisition racing declaration removal, type change, primary-origin change, alias-only reload, health-policy reload, remove/re-add ABA, pause, and Resume in both commit orders either commits against the exact revalidated declaration authority and pause state or restarts/fails without installing stale authority; pause-first acquisition installs only a paused probe-only scope, while Resume-first acquisition observes the unpaused state;
- an alias-only or unrelated reload transfers a lease and any route authorization only after proving the routing-relevant declaration unchanged; a health-policy reload preserves the lease and binding but withdraws authorization and reprobes; declaration removal, type change, primary-origin change, and remove/re-add retire authority; and route selection rejects any mismatch in admitted generation, published type, primary origin, policy, or canonical host;
- the authority-scoped readiness wait succeeds only for current route authorization on the exact lease handle and expected binding revision; remains pending through checking, unhealthiness, renewal, alias-only transfer, and health-policy reprobe; terminates on timeout, cancellation, replacement, rebind, expiry, release, pause, declaration or primary-origin invalidation, and daemon restart; and never mistakes a successor's `ready` status for the caller's binding;
...
- every rebind attempt reserves a fresh nonce before candidate work; concurrent exact duplicates join one probe and compare-and-swap and receive one terminal result; the successful binding swap and terminal record commit atomically; exact replay of a lost committed response returns the installed revision only while that binding remains current; exact replay of a recorded failure returns that failure without reprobing; mismatched reuse is rejected; and neither an old nonce nor a stale expected revision can observe or mutate a successor;
...
- acquisition and renewal set their deadline to their own commit time plus TTL and return server-selected relative `renew_after` and `expires_in` values; a publisher anchoring `renew_after` to its monotonic request-start renews conservatively despite delayed responses and renews immediately when that target has passed; a lost renewal response retried on the same active handle produces a fresh commit-time deadline and schedule; acquisition replay recomputes remaining time without renewing, returns an immediate-renew schedule while authority remains active but its normal margin is gone, and reports expiry only after the deadline elapses or authority retires; renewal never adds TTL to the prior deadline; wall-clock jumps do not affect lease authority; suspend time counts against the monotonic deadline; wake before or after a deadline, while paused, during Resume, configuration transfer or revocation, or candidate rebind serializes before acting on the registry and fences all pre-suspend work; every still-active binding is reproved in a fresh probe-only scope; and inability to establish suspend-inclusive elapsed time retires all publication authority, terminates exact waits, and requires fresh-nonce reacquisition;
