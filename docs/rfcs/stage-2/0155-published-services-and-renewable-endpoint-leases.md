...
3. The daemon binds the kernel credential PID to a high-resolution process-birth identity through the platform proof above: accepted-socket `SO_PEERPIDFD` on Linux, or the client audit-token prefix plus repeated `LOCAL_PEERTOKEN` equality on macOS. Numeric PID lookup alone is never sufficient.
...
- Version 1 binds process birth to the accepted peer generation with `SO_PEERPIDFD` on Linux 6.5 or newer and a fixed client audit-token transport proof on macOS; neither platform falls back to a later numeric-PID lookup.
