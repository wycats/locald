# Design: Advanced Proxying & Traffic Management

> **Core focus**: The default proxy behavior should remain simple and reliable.
...
To support modern web frameworks (Vite, Astro, Next.js) that use lazy compilation, the proxy implements a "Loading State" mechanism.

- **Problem**: These frameworks open their port immediately but hang on the first request while compiling.
- **Solution**: If a request to a service takes longer than **500ms** and accepts HTML, the proxy intercepts it.
- **UX**: The user sees a "Building..." page with a spinner instead of a hanging browser tab.
- **Mechanism**: The page polls the backend using a special `X-Locald-Passthrough` header. When the backend is ready, the page reloads automatically.
