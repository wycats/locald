...
- `image: Option<String>`: The Docker image to run (e.g., `postgres:15`). If present, `command` is ignored (or used as args?). *Decision: If `image` is present, `command` is optional and treated as the container command/args.*
- `container_port: Option<u16>`: The port *inside* the container to expose. Required if we want to map a port.
...
- **Robust Cleanup**:
    ...
    - On `stop()`, the `docker` CLI process is killed. Since we use `--rm`, the container *should* die, but we should also explicitly `docker stop` it if possible, or rely on the startup cleanup of the next run.
...
- **Host Access**: Containers often need to talk to the host (e.g., to reach another service running locally).
    ...
    - *Note*: On Linux, `host.docker.internal` is not always available by default. We might need `--add-host host.docker.internal:host-gateway`.
...
- The `<random_host_port>` is injected into the environment of *other* services as `PORT_<SERVICE_NAME>`.
