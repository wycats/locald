...
- **App Builder ("Regular Joe")**:
  ...
  - **Cons**: If they _already_ have a `docker-compose.yml` (common in existing projects), they might feel forced to migrate or maintain two configs.
...
- **Axiom 4: Process Ownership**:
  ...
  - **Risk**: `docker` client process is just a client. The actual container runs in the Docker daemon. If `locald` crashes, the `docker` CLI process dies, but does the container die?
    - _Mitigation_: `docker run --rm` usually handles this if the client disconnects, but it's not guaranteed. We might need a "reaper" logic on startup to clean up tagged containers.
...
- **The "localhost" Trap**:
  ...
  - _Solution_: This is a known Docker issue. On Linux, `--network host` works but isolates ports less. On Mac/Windows, `host.docker.internal` is needed. `locald` might need to inject a special env var like `HOST_URL` that resolves correctly depending on the OS.
- **Volume Permissions**:
  ...
  - _Mitigation_: This is out of scope for `locald` to fix entirely, but we should document it or provide reasonable defaults.
- **Zombie Containers**:
  - If `locald` is `kill -9`'d, the `docker run` process dies. Does Docker kill the container?
  - _Test required_: We need to verify if `--rm` is sufficient or if we need to explicitly name containers (`locald-<project>-<service>`) and kill them on startup (Axiom 4 implies we clean up before starting).
...
- **Hybrid Proposal**:
  ...
  - _Don't_ try to support every Docker feature (networks, complex volumes, healthchecks) in the schema. If they need that, they should use `command = "docker-compose up"` (which we already support as a raw command!).
