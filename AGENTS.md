...
Exosuit state is the source of truth for project state. We never guess; we read state through the best available Exosuit interface for the environment.

- **Implication**: Every session starts by reading Exosuit state through the available extension, machine-channel, LM-tool, or terminal interface. Every state mutation goes through an Exosuit interface rather than hand-editing generated state.
...
- **Read First**: Before answering, inspect Exosuit status and task state through the best available Exosuit interface for the environment.
- **Write Often**: Use Exosuit task, goal, idea, and inbox operations to keep state current.

### Protocol: Tool Usage

- **Structured IO**: When reading or modifying plan state, prefer structured Exosuit surfaces such as the VS Code extension, machine-channel, or LM tools. Use the `exo` CLI as a terminal/developer fallback when those surfaces are unavailable. State is stored in SQLite; Exosuit state, not any particular interface, is the source of truth.
...
- Exosuit interfaces: VS Code extension, machine-channel, LM tools, and terminal CLI surfaces over the same project state.
<!-- core end -->
